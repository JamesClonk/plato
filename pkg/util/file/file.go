...
func Checksum(file, expectedSHA string) {
	...
	case 40:
		sha1 := SHA1(file)
		if sha1 != expectedSHA {
			log.Fatalf("SHA1 checksum mismatch [%s]! actual:[%s], expected:[%s]",
				color.Cyan(file), color.Red(sha1), color.Green(expectedSHA))
		}
	case 64:
		sha256 := SHA256(file)
		if sha256 != expectedSHA {
			log.Fatalf("SHA256 checksum mismatch for [%s]! actual:[%s], expected:[%s]",
				color.Cyan(file), color.Red(sha256), color.Green(expectedSHA))
		}
	default:
		log.Fatalf(color.Red("Valid SHA checksum missing for [%s]", file))
	}
}
