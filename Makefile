...
test: trust
	SOPS_AGE_KEY_FILE="$$PWD/_fixtures/age.key" TZ="" go test -tags=faketime -v -race -p 1 ./...
	@echo "{{{ .ssh.public_key -}}}" | PLATO_WORKING_DIR=_fixtures/combined SOPS_AGE_KEY_FILE=../age.key go run -race main.go template | grep -q 'AAAAC3NzaC1lZDI1NTE5AAAAIMmROMaltL5DRYCYNHW9BE2BTZWUqZ3dISyzZopyrRjg' || (echo "failed basic stdin/stdout test" && exit 1)
